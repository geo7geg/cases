...
namespace WebApplication1
{
    public partial class events : System.Web.UI.Page
    {
        ...
        {
            if (event_date.Text != "" && cases.Text != "")
            {
                MySqlConnection connection = new MySqlConnection(connectionString);
                connection.Open();
                MySqlCommand command = connection.CreateCommand();

                command.CommandText = "UPDATE event SET date=@p1, Cases=@p2, Aiton=@p3, Perigrafi=@p4, final_date=@p5 WHERE ID=@p21";
                command.Prepare();
                ...
                command.Parameters.AddWithValue("@p4", perigrafi.Text);
                command.Parameters.AddWithValue("@p5", final_date.Text);
